...
//读取文件数据，创建图
void create_graph() {
    ifstream infile("/Users/travis/CPPProjects/tabucol-master/data/simple5.txt", ios::in);
    vector<string> data;
    string delim(" ");
    string textline;
    bool start = false;
...
void initialization(int numofcolor) {
    ...
    for (int i = 0; i<N; i++) {
        if (fixedColor[i] == -1) {
            ...
            // sort(vv[vvlocation].begin(), vv[vvlocation].end());
            // sort(vvline[vvlinelocation].begin(), vvline[vvlinelocation].end());
...
void tabusearch() {
    ...
    N12 = numofcolor;
    create_graph();
    ofstream ofile("/Users/travis/CPPProjects/tabucol-master/simple5result.txt", ios::out);
    ...
    while (f>0) {
        iter++;
        if ((iter % 100000) == 0) std::cout << iter / 100000 << " --- " << f << endl;
